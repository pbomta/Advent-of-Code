...
class burrow:
    ...
    def __str__(self):
        ...
        for h in self.hallway:
            if h.isnumeric():
                result += self.anthropodStr[int(h)]
            else:
                result += h
        result += "\n "
        for b in range(len(self.burrows)):
            if len(self.burrows[b]) == 2:
                result += " " + self.anthropodStr[int(self.burrows[b][1])]
            else:
                result += "  "
        result += "  \n "
        for b in range(len(self.burrows)):
            if len(self.burrows[b]) > 0:
                result += " " + self.anthropodStr[int(self.burrows[b][0])]
            else:
                result += "  "
        result += "  \n"
        return result
    ...
    #     # for p in range(len(self.hallway)):
    #     #     if self.hallway[p] != other.hallway[p]:
    #     #         return False
    #     # for b in range(len(self.burrows)):
    #     #     if self.burrows[b] != other.burrows[b]:
    #     #         return False
    #     # return True
    ...
    def moveToBurrow(self, pos): # pos in hallway
        ...
        a = int(self.hallway[pos]) # Anthropod

        if self.burrows[a] == "":
            steps = 2
        elif self.burrows[a] == str(a):
            steps = 1
        else:
        ...
        result = burrow(self.hallway[:pos] + '.' + self.hallway[pos+1:], self.burrows[:a] + tuple(self.burrows[a] + str(a)) + self.burrows[a+1:])
        assert(len(result.burrows[a]) <= 2)
        result.energy_usage = self.energy_usage + steps * pow(10, a)
        return result

    def moveToHallway(self, b): # b = burrow index
        if self.burrows[b] == "":
            return None # Empty burrow
        if self.burrows[b] == str(b):
            return None # or only filled with the correct anthropods
        if self.burrows[b] == str(b) + str(b):
            return None
        a = int(self.burrows[b][-1])
        ...
        while p > 0:
            ...
            new_hallway = self.hallway[:p] + str(a) + self.hallway[p+1:]
            new_burrow = self.burrows[:b] + tuple(self.burrows[b][:-1]) + self.burrows[b+1:] # FIXME: dit werkt niet!!! Een lege tuple wordt weggegooid onder alle omstandigheden. Wat een rot software, dat Python.
            new_state = burrow(new_hallway, new_burrow)
            new_state.energy_usage = self.energy_usage + steps * pow(10, a)
            print(new_state.burrows)
            assert(len(new_state.burrows) == 4)
        ...
        while p < len(self.hallway) - 1:
            ...
            if p in self.burrowsPos:
                continue # Move beyond burrow exits
            new_state = burrow(self.hallway[:p] + str(a) + self.hallway[p+1:], self.burrows[:b] + tuple(self.burrows[b][:-1]) + self.burrows[b+1:])
            new_state.energy_usage = self.energy_usage + steps * pow(10, a)
            assert(len(new_state.burrows) == 4)
            result.append(new_state)
        return result
...
while frontier:
    ...
    frontier.sort(key=lambda B: B.energy_usage, reverse=True)
